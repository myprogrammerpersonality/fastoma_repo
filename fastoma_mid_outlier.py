...
class HOG:
    ...
    def to_orthoxml(self,**gene_id_name): #, indent=0):
        ...
        if len(self._subhogs) == 0:
            ...
            geneRef_elemnt = ET.Element('geneRef', attrib={'id': str(gene_id_name[list_member_first] )}) # # gene_id_name[query_prot_record.id]
...
def distribute_rhogs(rhogs: List[Tuple[str, int]], start_index: int, n_workers: int) -> List[Tuple[str, int]]:
    """Distribute rhogs to workers in a way that each worker gets both big and small rhogs.
    inputs:
        rhogs: a list of tuples of (rhogs name, rhogs size in bytes)
        start_index: the index of the worker
        n_workers: the number of workers
    outputs:
        a list of tuples of (rhogs name, rhogs size in bytes)"""
